...
__global__ void Mean_Blur_CUDA_kernel(unsigned char* Dev_Input_Image, unsigned char* Dev_Output_Image);

unsigned char* Mean_Blur_CUDA(unsigned char* Input_Image, int Height, int Width) {
    ...
    Mean_Blur_CUDA_kernel << <Grid_Image, Block_size, shm_size >> > (Dev_Input_Image, Dev_Output_Image);
